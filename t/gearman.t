...
sub test_run_locally()
{
    ...
    {
        eval { FailsAlwaysWorker->run_locally( { 'foo' => 'bar' } ); };
        ok( $@, 'run_locally() FailsAlwaysWorker' );
    }

    {
        # 1
        eval { FailsOnceWorker->run_locally( { 'foo' => 'bar' } ); };
...
sub test_run_on_gearman()
{
    ...
    {
        eval { FailsAlwaysWorker->run_on_gearman( { 'foo' => 'bar' } ); };
        ok( $@, 'run_on_gearman() FailsAlwaysWorker' );
    }

    {
        # 1
        eval { FailsOnceWorker->run_on_gearman( { 'foo' => 'bar' } ); };
...
sub test_enqueue_on_gearman()
{
...

    {
        ...
        is(
            read_file( $write_results_to ),
            reverse( $string ),
            'enqueue_on_gearman() ReverseStringWorker result string matches'
        );
    }

    {
        ...
        my $started = FailsOnceWillRetryWorker->enqueue_on_gearman( { 'write_results_to' => $write_results_to } );
